...
class MainWindow(QMainWindow):
    ...
    def a_downloadRequested(self,item):  # QWebEngineDownloadItem
        print('downloading to', item.path())
        item.accept()
        item.finished.connect(self.foo)
    ...
    def _downloadRequested(self, download):
        ...
        path, _ = QtWidgets.QFileDialog.getSaveFileName(None, "Save File", old_path, "*." + suffix)
        if path:
